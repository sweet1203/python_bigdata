...
export default function AssessmentShell({
  ...
  const isStudentStep = currentSlug && assessmentPages.some((p) => p.slug === currentSlug);
      ...
      <div className="flex flex-col gap-8 lg:flex-row lg:items-start">
        <aside className="hidden w-64 shrink-0 lg:block">
          <div className="sticky top-6 space-y-4">
            <Link
              href="/assessment"
              className="block text-sm font-semibold text-amber-800 hover:text-amber-950"
            >
              ← 수행평가 안내 홈
            </Link>
        ...
        <main className="min-w-0 flex-1">
          <article className="space-y-6 rounded-xl border border-amber-200/70 bg-gradient-to-b from-white to-amber-50/20 p-6 shadow-sm lg:p-8">
            ...
            {isStudentStep && (
              <AssessmentStepFooter slug={currentSlug as AssessmentSlug} />
            )}

            {!currentSlug && (
              <p className="border-t border-slate-100 pt-4 text-center text-sm text-slate-500">
                권장 순서:{" "}
                {assessmentPages.map((p, i) => (
                  <span key={p.slug}>
                    {i > 0 && " → "}
                    <Link href={p.path} className="font-medium text-primary-700 hover:underline">
                      {p.title}
                    </Link>
                  </span>
                ))}
